...
float Processor::Utilization() {
  ...
  long int usertime = std::stol(cpu_usage[0]);
  long int nicetime = std::stol(cpu_usage[1]);
  long int systemtime = std::stol(cpu_usage[2]);
  long int idletime = std::stol(cpu_usage[3]);
  long int iowait = std::stol(cpu_usage[4]);
  long int irq = std::stol(cpu_usage[5]);
  long int softirq = std::stol(cpu_usage[6]);
  long int steal = std::stol(cpu_usage[7]);

  // Calculate needed jiffy values (current state)
  float IDLE = idletime + iowait;
  float NON_IDLE = usertime + nicetime + systemtime + irq + softirq + steal;
  ...
  if (prevuser == -1 || prevnice == -1 || prevsystem == -1 || previdle == -1 ||
    ...
    long int PREV_IDLE = previdle + previowait;
    long int PREV_NON_IDLE =
    ...
    double IDLE_DIF = IDLE - PREV_IDLE;
